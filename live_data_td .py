            ...
            if self.wait_for_data(softCutoff, hardCutoff):
                ...
                for _ in range(self.burstLimit):
                    ...
                    try:
                        ...
                        continue
                    whitelist_match = list(
                        filter(lambda x: x.get('software').lower() == software.lower(), config['whitelist']))
                    # Upload software not on whitelist is ignored.
                    if len(whitelist_match) == 0:
                        if config['debug'] or config['verbose']:
                            print(
                                system + "/" + station + " update rejected from client not on whitelist: " + software + " v" + swVersion + "\n")
                        continue
...
#             print("Update checker is sleeping: " + str(int(now + config['check_update_every_x_sec'] - round(time.time(),
#                                                                                                             0))) + " seconds remain until next check.")
#         time.sleep(1)
#
#     # If not go:
...
#         plugin_options = plugins.load(cmdenv(['trade', 'import', '--plug', 'eddblink', '-O', 'help']).plug,
#                                       "ImportPlugin").pluginOptions.keys()
#
...
#
#         if config['verbose']:
#             print("Updated " + system + "/" + station + ", station_id:'" + str(
#                 station_id) + "', from " + software + " v" + swVersion)
#         else:
...
#             try:
#                 cursor = fetchIter(
#                     db_execute(db, "SELECT * FROM StationItem WHERE from_live = 1 ORDER BY station_id, item_id"))
#                 results = list(cursor)
...
#                     collected_at = str(
#                         timegm(datetime.datetime.strptime(result[8].split('.')[0], '%Y-%m-%d %H:%M:%S').timetuple()))
...
#             if int(station['type_id']) == 19 or int(station['type_id']) == 24 or int(
#                     station['unq:station_id']) == 42041:
#                 full_name = "MEGASHIP"
#             else:
