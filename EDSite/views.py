...
def systems(request):
    ...
    if request.method == "GET":
        ...
        if ref_system_name_or_id and ref_system_name_or_id.isdigit():
            ...
            ref_system = System.objects.filter(
                name__icontains=ref_system_name_or_id
            ).first()

        filtered_systems = System.objects.order_by("id")

        if only_populated:
            filtered_systems = filtered_systems.annotate(
                num_stations=Count("stations")
            ).filter(num_stations__gt=0)

    filtered_systems = filtered_systems[:40]
    if ref_system:
        print("Reference: ", ref_system)
        distances = {
            other_system.id: int(other_system.distance_to(ref_system))
            for other_system in filtered_systems
        }
        filtered_systems = sorted(
            list(filtered_systems),
            key=lambda filtered_system: distances[filtered_system.id],
            reverse=False,
        )
    ...
    context["form"] = form
    return render(
        request, "EDSite/system/systems.html", base_context(request) | context
    )
...
def stations(request):
    ...
    if request.method == "GET":
        ...
        if ref_system_name_or_id and ref_system_name_or_id.isdigit():
            ...
            ref_system = (
                System.objects.filter(name__icontains=ref_system_name_or_id)
                .order_by("id")
                .first()
            )
            if ref_system:
                filtered_stations = Station.objects.select_related("system").order_by(
                    "id"
                )
            else:
                filtered_stations = Station.objects.order_by("id")
        ...
        if star_distance and star_distance.isdigit():
            filtered_stations = filtered_stations.filter(
                Q(ls_from_star__lte=int(star_distance))
            )
    ...
    if ref_system:
        ...
        distances = {
            other_station.id: int(other_station.system.distance_to(ref_system))
            for other_station in filtered_stations
        }
        filtered_stations = sorted(
            list(filtered_stations),
            key=lambda filtered_station: distances[filtered_station.id],
            reverse=False,
        )
        if system_distance:
            filtered_stations = filter(
                lambda filtered_station: distances[filtered_station.id]
                <= int(system_distance),
                list(filtered_stations),
            )
        context["reference_distances"] = distances
    ...
    context["form"] = form
    return render(
        request, "EDSite/station/stations.html", base_context(request) | context
    )
...
def station(request, station_id):
    ...
    context = {"station": station, "listings_by_category": listings_by_category}
    return render(
        request, "EDSite/station/station.html", base_context(request) | context
    )
...
def commodities(request):
    context = {
        "categories": sorted(
            CommodityCategory.objects.all(),
            key=lambda category: max(
                category.commodities.all(), key=lambda com: com.max_profit
            ).max_profit,
            reverse=True,
        )
    }
    return render(
        request, "EDSite/commodity/commodities.html", base_context(request) | context
    )
...
def commodity(request, commodity_id):
    ...
    if request.method == "GET":
        ...
        if form.is_valid():
            ...
            landing_pad_size = form.data.get("landing_pad_size")
            minimum_units = (
                form.data.get("minimum_units") if form.data.get("minimum_units") else 0
            )
            ...
            if buy_or_sell == "sell":
                filtered_listings = LiveListing.objects.filter(
                    commodity_id=commodity_id
                ).filter(Q(demand_units__gt=minimum_units))
            else:
                filtered_listings = LiveListing.objects.filter(
                    commodity_id=commodity_id
                ).filter(Q(supply_units__gt=minimum_units))
            if not include_planetary:
    ...
    filtered_listings = filtered_listings.order_by(
        ordering
    )  # TODO: Performance. This makes it slow.
    filtered_listings = filtered_listings[:40]
    if ref_system:
        filtered_listings = filtered_listings.select_related("station__system")
        distances = {
            listing.id: int(listing.station.system.distance_to(ref_system))
    ...
    context["form"] = form
    return render(
        request, "EDSite/commodity/commodity.html", base_context(request) | context
    )
...
def rares(request):
    context = {}
    return render(
        request, "EDSite/commodity/rares.html", base_context(request) | context
    )
...
def carrier_missions(request, tab=None):
    ...
    context = {"tab": tab, "all_missions": [], "my_missions": [], "grid_cols": range(3)}
    if tab == "all":
        missions: [CarrierMission] = CarrierMission.objects.all()
        context["all_missions"] = list_to_columns(missions, 3)
        context["mission_distances"] = {
            mission.id: int(mission.station.system.distance_to(CURRENT_SYSTEM))
        ...
        if request.method == "GET":
            ...
            if base_context_local.get("current_user"):
                mission_form.fields["username"].initial = base_context_local.get(
                    "current_user"
                ).username

            if base_context_local.get("current_carrier"):
                mission_form.fields["carrier_name"].initial = base_context_local.get(
                    "current_carrier"
                )[0]
            ...
            if mission_form.is_valid():
                ...
                if not station_id or not commodity_id or not carrier_id:
                    print(
                        f"CarrierMissionForm choices are not valid: station_id={station_id}, commodity_id={commodity_id}, carrier_id={carrier_id}"
                    )
                else:
                    station = Station.objects.get(pk=station_id)
    ...
    return render(
        request,
        "EDSite/carrier_missions/carrier_missions_base.html",
        base_context_local | context,
    )
...
def debug_reload(request):
    ...
    # EDData().update_tradedangerous_database()
    return JsonResponse({"status": EDData().td_database_status.__str__()})
...
def debug_update_database(request, mode="False"):
    ...
    return JsonResponse({"status": "No status"})
...
def signup_view(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            user = (
                form.save()
            )  # authenticate(username=username, password=password1, email=email)
            login(request, user)
            return redirect("index")
    ...
    return render(
        request=request,
        template_name="EDSite/account/signup.html",
        context=base_context(request) | {"signup_form": form},
    )
...
def login_view(request):
    ...
    return render(
        request=request,
        template_name="EDSite/account/login.html",
        context=base_context(request) | {"login_form": form},
    )
